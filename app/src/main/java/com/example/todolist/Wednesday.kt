...
import kotlinx.android.synthetic.main.wednesday.view.*

class Wednesday : Fragment() ,ItemAdapter.OnItemClickListener {
    ...
    override fun onCreateView(
        ...
        var list = db?.ReadData("Wednesday")
        if (list != null) {
            mainlist = list
        }
        mainlist = mainlist.sortedBy { it.DateStart }
        val view = inflater.inflate(R.layout.wednesday, container, false)
        view.WednesdayItems.layoutManager = LinearLayoutManager(activity)
        view.WednesdayItems.adapter = mainlist?.let { ItemAdapter(it,this) }
        if (list != null) {
            if(list.isNotEmpty()){
    ...
    override fun onItemClick(position: Int) {
        ...
        ToDoActivity.putExtra("TABLE_NAME", "Wednesday")
