...
import kotlinx.android.synthetic.main.friday.view.*

class Friday : Fragment(), ItemAdapter.OnItemClickListener{
    ...
    override fun onCreateView(
        ...
        var list = db?.ReadData("Friday")
        if (list != null) {
            mainlist = list
        }
        mainlist = mainlist.sortedBy { it.DateStart }
        val view = inflater.inflate(R.layout.friday, container, false)
        view.FridayItems.layoutManager = LinearLayoutManager(activity)
        view.FridayItems.adapter = mainlist?.let { ItemAdapter(it,this) }
        if (list != null) {
            if(list.isNotEmpty()){
    ...
    override fun onItemClick(position: Int) {
        ...
        ToDoActivity.putExtra("TABLE_NAME", "Friday")
