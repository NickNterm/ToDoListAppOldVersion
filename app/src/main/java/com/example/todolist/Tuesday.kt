...
import kotlinx.android.synthetic.main.tuesday.view.*


class Tuesday : Fragment() ,ItemAdapter.OnItemClickListener {
    ...
    override fun onCreateView(
        ...
        var list = db?.ReadData("Tuesday")
        if (list != null) {
            mainlist = list
        }
        mainlist = mainlist.sortedBy { it.DateStart }
        val view = inflater.inflate(R.layout.tuesday, container, false)
        view.TuesdayItems.layoutManager = LinearLayoutManager(activity)
        view.TuesdayItems.adapter = mainlist?.let { ItemAdapter(it,this) }
        if (list != null) {
            if(list.isNotEmpty()){
    ...
    override fun onItemClick(position: Int) {
        ...
        ToDoActivity.putExtra("TABLE_NAME", "Tuesday")
